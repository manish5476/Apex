export interface PermissionDefinition {
  ...
  const p = (tag: string, group: string, description: string): PermissionDefinition => ({ tag, group, description });
  ...
  export const PERMISSIONS_LIST: PermissionDefinition[] =  [
    ...
    p("platform:read", "Admin Platform", "View enterprise admin dashboard and platform summaries"),
    p("platform:manage", "Admin Platform", "Manage admin platform operations"),
    p("platform:user_manage", "Admin Platform", "Block, unblock, suspend, approve, and inspect users"),
    p("platform:role_manage", "Admin Platform", "Assign roles and inspect permission matrix"),
    p("platform:settings", "Admin Platform", "Manage runtime platform settings"),
    p("platform:feature_flags", "Admin Platform", "Manage feature flags and rollout rules"),
    p("platform:security", "Admin Platform", "Review suspicious activity and account lock events"),
    p("platform:audit", "Admin Platform", "View admin platform audit logs"),
    p("platform:impersonate", "Admin Platform", "Generate short-lived user impersonation sessions"),
    p("platform:internal_tools", "Admin Platform", "Access hidden internal developer tools"),
  ...
  export const VALID_TAGS: string[] = [
    "*",
    ...PERMISSIONS_LIST.map((x) => x.tag),
    ...[...new Set(PERMISSIONS_LIST.map((x) => x.tag.split(":")[0]))].map((cat) => `${cat}:*`),
  ];
  
  // Reconstruct the dynamic PERMISSIONS object strictly typed
  export const PERMISSIONS = PERMISSIONS_LIST.reduce((acc, { tag }) => {
    const [resource, ...rest] :any= tag.split(":");
    const rKey = resource.toUpperCase().replace(/-/g, "_");
    const aKey = rest.join("_").toUpperCase().replace(/-/g, "_");
  ...
  export const mergePermissions = (
    ...
    overrides?.granted?.forEach((perm) => base.add(perm));
    overrides?.revoked?.forEach((perm) => base.delete(perm));
    return Array.from(base);
  };
  
  export const hasPermission = (userPerms: string[] | undefined, required: string): boolean => {
    if (!userPerms?.length) return false;
    if (userPerms.includes("*")) return true;
    if (userPerms.includes(required)) return true;
    const [cat] = required.split(":");
    return userPerms.includes(`${cat}:*`);
  };
  
  export const hasAnyPermission = (userPerms: string[], required: string[]): boolean =>
    required.some((r) => hasPermission(userPerms, r));
  
  export const hasAllPermissions = (userPerms: string[], required: string[]): boolean =>
    required.every((r) => hasPermission(userPerms, r));
  
  export const getPermissionsByGroup = (group: string): PermissionDefinition[] =>
    PERMISSIONS_LIST.filter((p) => p.group === group);
  
  export const getPermissionGroups = (): string[] =>
    Array.from(new Set(PERMISSIONS_LIST.map((p) => p.group)));