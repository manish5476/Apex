...
exports.updateMyProfile = catchAsync(async (req, res, next) => {
  ...
  Object.keys(req.body).forEach(key => {
    if (allowedFields.includes(key) || key.startsWith('preferences.') || key.startsWith('employeeProfile.')) {
      if (key === 'employeeProfile.employeeId') return; // double-block via prefix match
      filteredBody[key] = req.body[key];
    }
  });

  // Validate + clean secondary phone if provided
  const secondaryPhone = filteredBody['employeeProfile.secondaryPhone']
    ?? filteredBody.employeeProfile?.secondaryPhone;

  if (secondaryPhone) {
    if (!validatePhone(secondaryPhone))
      return next(new AppError('Please provide a valid secondary phone number', 400));
    filteredBody['employeeProfile.secondaryPhone'] = cleanPhone(secondaryPhone);
...
exports.getAllUsers = catchAsync(async (req, res, next) => {
  ...
  // Translate human-readable query params → DB field paths
  ...
  if (req.query.designation) {
    const desig = await Designation.findOne({ title: req.query.designation, organizationId: req.user.organizationId });
...
exports.createUser = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    if (employeeProfile?.departmentId) {
      const ok = await Department.exists({ _id: employeeProfile.departmentId, organizationId: orgId });
      if (!ok) throw new AppError('Invalid Department ID.', 400);
    }
    if (employeeProfile?.designationId) {
      const ok = await Designation.exists({ _id: employeeProfile.designationId, organizationId: orgId });
    ...
    res.status(201).json({
      status: 'success',
      data: { user: newUser, message: 'User created. They must reset their password on first login.' },
    });
...
exports.updateUser = catchAsync(async (req, res, next) => {
  ...
  if (updatePayload['employeeProfile.reportingManagerId']) {
    const ok = await User.exists({ _id: updatePayload['employeeProfile.reportingManagerId'], organizationId: req.user.organizationId });
    if (!ok) return next(new AppError('Reporting Manager not found.', 400));
  }
  if (updatePayload['employeeProfile.departmentId']) {
    const ok = await Department.exists({ _id: updatePayload['employeeProfile.departmentId'], organizationId: req.user.organizationId });
    if (!ok) return next(new AppError('Department not found.', 400));
  }
  if (updatePayload['employeeProfile.designationId']) {
    const ok = await Designation.exists({ _id: updatePayload['employeeProfile.designationId'], organizationId: req.user.organizationId });
