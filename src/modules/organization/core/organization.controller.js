...
/* ---------------------------------------------------------------
 * CREATE ORGANIZATION
 * POST /organization/create  (public)
--------------------------------------------------------------- */
exports.createOrganization = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    const newOrg = new Organization({
      ...
      settings: { currency: 'INR', timezone: 'Asia/Kolkata', financialYearStart: 'April' },
    });
    ...
    // Save sequentially inside the transaction
    ...
    res.status(201).json({
      ...
      token: accessToken,       // kept for backward compat with frontend
      refreshToken,
      data: {
        organization: newOrg,
        owner: newOwner,
    ...
    try {
      ...
      console.warn('Transaction already aborted:', abortErr.message);
    }
...
/* ---------------------------------------------------------------
 * GET PENDING MEMBERS
 * GET /organization/pending-members
--------------------------------------------------------------- */
...
/* ---------------------------------------------------------------
 * APPROVE MEMBER
 * POST /organization/approve-member
--------------------------------------------------------------- */
exports.approveMember = catchAsync(async (req, res, next) => {
  ...
  // Validate role and branch belong to this org
...
/* ---------------------------------------------------------------
 * REJECT MEMBER
 * POST /organization/reject-member
--------------------------------------------------------------- */
exports.rejectMember = catchAsync(async (req, res, next) => {
  ...
  if (!user) return next(new AppError('Pending request not found.', 404));
  ...
  if (typeof emitToOrg === 'function') {
    emitToOrg(req.user.organizationId, 'memberRejected', {
      userId,
      message: `Request for ${user.name} was rejected.`,
    });
  }
...
/* ---------------------------------------------------------------
 * PUBLIC: Verify Shop ID (for login screen)
 * GET /organization/shop/:uniqueShopId
--------------------------------------------------------------- */
...
/* ---------------------------------------------------------------
 * PUBLIC: Lookup Organizations by Email
...
exports.lookupOrganizations = catchAsync(async (req, res, next) => {
  ...
  if (users && users.length > 0) {
    const orgs = users
      ...
      .map(u => ({ name: u.organizationId.name, shopId: u.organizationId.uniqueShopId }));
    ...
    if (uniqueOrgs.length > 0) {
      ...
      await sendEmail({
        ...
        html: `
          <div style="font-family:'Segoe UI',Tahoma,sans-serif;max-width:600px;margin:auto;padding:20px;border:1px solid #eee;border-radius:10px;">
            <h2 style="color:#2c3e50;border-bottom:2px solid #f1c40f;padding-bottom:10px;">Organization Details Found</h2>
            <p>Hello,</p>
            <p>Here are the organizations associated with this email:</p>
            <ul style="list-style:none;padding:0;">${orgListHtml}</ul>
            <p style="margin-top:20px;">Use these <strong>Shop IDs</strong> to log in at the portal.</p>
            <p style="color:#7f8c8d;font-size:.9em;">If you didn't request this, please ignore this email.</p>
            <hr style="border:0;border-top:1px solid #eee;margin:30px 0;"/>
            <p style="font-size:.8em;text-align:center;color:#bdc3c7;">&copy; ${new Date().getFullYear()} Apex CRM. All rights reserved.</p>
          </div>
        `,
      });
    }
  ...
  res.status(200).json({
    status: 'success',
    message: 'If an account is associated with this email, you will receive your organization details shortly.',
  });
});

/* ---------------------------------------------------------------
 * UPDATE MY ORGANIZATION (self-service)
 * PATCH /organization/my-organization
--------------------------------------------------------------- */
exports.updateMyOrganization = catchAsync(async (req, res, next) => {
  const orgId = req.user.organizationId;
  if (!orgId) return next(new AppError('You are not linked to any organization.', 400));

  // Only owner can change uniqueShopId or gstNumber
  if (!req.user.isOwner && (req.body.uniqueShopId || req.body.gstNumber))
    return next(new AppError('Only the organization owner can change the Shop ID or GST number.', 403));
  ...
  // Nested: address (dot notation for safe partial update)
  ...
  if (req.body.settings && typeof req.body.settings === 'object') {
    ['currency', 'timezone', 'financialYearStart'].forEach(field => {
  ...
  if (!updatedOrg) return next(new AppError('Organization not found.', 404));
...
/* ---------------------------------------------------------------
 * DELETE MY ORGANIZATION (owner only)
 * DELETE /organization/my-organization
--------------------------------------------------------------- */
exports.deleteMyOrganization = catchAsync(async (req, res, next) => {
  ...
  await Organization.findByIdAndDelete(req.user.organizationId);
...
/* ---------------------------------------------------------------
 * GET MY ORGANIZATION (with live members)
 * GET /organization/my-organization
--------------------------------------------------------------- */
exports.getMyOrganization = catchAsync(async (req, res, next) => {
  ...
  // Members are sourced from the User collection, not the branches[] array
...
/* ---------------------------------------------------------------
 * GET ANY ORGANIZATION (platform admin)
 * GET /organization/:id
--------------------------------------------------------------- */
