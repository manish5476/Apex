...
// const imageService = require('../services/uploads/imageUploadService');
// const catchAsync = require('../../core/utils/api/catchAsync');

// /**
//  * Organization Gallery: Search and Filter all images
//  */
// exports.getOrgGallery = catchAsync(async (req, res) => {
...
//   const [assets, total] = await Promise.all([
//     Asset.find(query).sort({ createdAt: -1 }).skip(skip).limit(Number(limit)).populate('uploadedBy', 'name'),
//     Asset.countDocuments(query)
//   ]);
...
// /**
//  * Direct CRUD: Delete asset by ID
//  */
// exports.removeAsset = catchAsync(async (req, res) => {
//   await imageService.deleteFullAsset(req.params.id, req.user.organizationId);
//   res.status(204).json({ status: 'success', message: 'Asset permanently removed' });
// });
...
// // exports.getMyMediaGallery = catchAsync(async (req, res) => {
...
// // // GET STORAGE STATS (How much space is Shivam Electronics using?)
...
// //   res.status(200).json({ status: 'success', data: stats[0] || { totalSize: 0, totalFiles: 0 } });
// // });