...
// Keep LangChain dependencies global but initialize per-request to ensure tenant isolation
let langChainDeps = null;

async function initLangchainDeps() {
  ...
  try {
    ...
    const prompt = ChatPromptTemplate.fromMessages([
      ["system", "You are an expert CRM assistant. Use tools when data required. If tools return JSON, summarise them in a human-readable format."],
      ["human", "{input}"],
      ["placeholder", "{agent_scratchpad}"],
    ]);

    langChainDeps = { llm, prompt, createToolCallingAgent, AgentExecutor, DynamicStructuredTool, z };
    debug("LangChain dependencies loaded");
    return langChainDeps;
  } catch (err) {
...
async function processUserMessage(message, userContext = {}) {
  const { organizationId, branchId } = userContext;
  if (!organizationId) return "organizationId required";

  const deps = await initLangchainDeps();
