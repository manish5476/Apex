...
class PurchaseService {
  ...
  static async createPurchase(data, user) {
    const {
      ...
      updateCostPrice = false, // ← opt-in: sync invoice price back to product catalog
    } = data;
    ...
    await runInTransaction(async (session) => {
      ...
      if (status === 'received') {
        ...
        // ── Opt-in: update product catalog purchasePrice ──────────────
        if (updateCostPrice) {
          await this._updateProductCostPrices(enrichedItems, user.organizationId, session);
        }
  ...
  static async partialReturn(purchaseId, { items: retItems, reason }, user) {
    ...
    await runInTransaction(async (session) => {
      ...
      // Bug fix: aggregate already-returned quantities for this purchase
      // so we cannot double-return more than was originally purchased.
      ...
      for (const r of priorReturns) {
        for (const i of r.items) {
          ...
          alreadyReturnedMap[key] = (alreadyReturnedMap[key] || 0) + i.quantity;
        }
      }
      ...
      for (const retItem of retItems) {
        // A. Validate against original invoice minus already-returned qty
        const original = purchase.items.find(
          i => String(i.productId) === String(retItem.productId)
        );
        const alreadyReturned = alreadyReturnedMap[String(retItem.productId)] || 0;
        const maxReturnable = original ? original.quantity - alreadyReturned : 0;

        if (!original || maxReturnable <= 0) {
          throw new AppError(
            `"${original?.name || retItem.productId}" has already been fully returned`, 400
          );
        }
        if (retItem.quantity > maxReturnable) {
          throw new AppError(
            `Return qty (${retItem.quantity}) exceeds remaining returnable qty (${maxReturnable}) ` +
            `for "${original.name}" — ${alreadyReturned} already returned previously`, 400
          );
        }
  ...
  static async _enrichAndCalculate(rawItems, organizationId, session = null) {
    ...
    const query = Product.find({
      ...
      organizationId,
      isActive: true,
      isDeleted: { $ne: true }, // Bug fix: block purchases against deleted/inactive products
    }).select('name');
  ...
  /**
   * Optionally update the product catalog purchasePrice when a purchase is received.
   * Only runs when the caller passes updateCostPrice=true.
   * Uses bulkWrite with $ne filter so only truly changed prices are written,
   * and runs inside the same Mongoose session so it rolls back on any failure.
   */
  static async _updateProductCostPrices(items, orgId, session) {
    const ops = items
      .filter(item => item.productId && item.purchasePrice != null)
      .map(item => ({
        updateOne: {
          filter: {
            _id: item.productId,
            organizationId: orgId,
            purchasePrice: { $ne: item.purchasePrice },
          },
