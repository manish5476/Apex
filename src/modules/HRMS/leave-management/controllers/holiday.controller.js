...
exports.createHoliday = catchAsync(async (req, res) => {
  ...
  const holiday = await holidayService.create(req.user.organizationId, payload, req.user._id);
...
exports.getHoliday = catchAsync(async (req, res, next) => {
  const holiday = await repo.getById(req.user.organizationId, req.params.id);
  if (!holiday) return next(new AppError('Holiday not found', 404));
  return success(res, { holiday });
});

exports.updateHoliday = catchAsync(async (req, res) => {
  const payload = updateHolidaySchema.parse(req.body);
  const holiday = await holidayService.update(req.user.organizationId, req.params.id, payload, req.user._id);
  return success(res, { holiday });
});

exports.deleteHoliday = catchAsync(async (req, res) => {
  await holidayService.delete(req.user.organizationId, req.params.id);
...
exports.getHolidaysByYear = catchAsync(async (req, res) => {
  ...
  const holidays = await repo.getByYear(req.user.organizationId, year, branchId);
  const byMonth = {};
  holidays.forEach(h => {
  ...
  return success(res, { year, total: holidays.length, byMonth, holidays });
});

exports.getUpcomingHolidays = catchAsync(async (req, res) => {
  const limit = Math.min(parseInt(req.query.limit) || 10, 50);
  const holidays = await repo.getUpcoming(req.user.organizationId, req.user.branchId, limit);
  return success(res, { holidays }, 200, { total: holidays.length });
});

exports.checkDate = catchAsync(async (req, res) => {
  const payload = checkDateSchema.parse(req.body);
  
  const holiday = await repo.findExistingDate(
  ...
  return success(res, { date: payload.date, isHoliday: !!holiday, holiday: holiday || null });
});

// --- Bulk Operations ---

exports.bulkCreateHolidays = catchAsync(async (req, res) => {
  const payload = bulkCreateSchema.parse(req.body);
  const results = await holidayService.bulkCreate(req.user.organizationId, payload, req.user._id);
  return created(res, results);
});

exports.copyHolidaysFromYear = catchAsync(async (req, res) => {
  const payload = copyYearSchema.parse(req.body);
  const holidays = await holidayService.copyFromYear(req.user.organizationId, payload.fromYear, payload.toYear, payload.branchId, req.user._id);
  return created(res, { fromYear: payload.fromYear, toYear: payload.toYear, copied: holidays.length, holidays });
});

// --- Analytics & Reports ---

exports.getHolidayStats = catchAsync(async (req, res) => {
  const year = parseInt(req.query.year) || new Date().getFullYear();
  const [stats] = await repo.getStatsAggregation(req.user.organizationId, year);
  return success(res, { year, stats });
});

exports.exportHolidays = catchAsync(async (req, res) => {
  const year = parseInt(req.query.year) || new Date().getFullYear();
  const branchId = req.query.branchId;
  const format = req.query.format || 'json';

  if (format === 'calendar') {
    const result = await holidayService.buildCalendar(req.user.organizationId, year, branchId);
    return success(res, result);
  }

  const holidays = await repo.getByYear(req.user.organizationId, year, branchId);
  return success(res, { year, total: holidays.length, holidays });
});
