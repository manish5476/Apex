...
exports.getAllLeaveBalances = catchAsync(async (req, res) => {
...
exports.getLeaveBalance = catchAsync(async (req, res, next) => {
  const leaveBalance = await repo.getById(req.user.organizationId, req.params.id);
  if (!leaveBalance) return next(new AppError('Leave balance not found', 404));
  return success(res, { leaveBalance });
});

exports.getMyLeaveBalance = catchAsync(async (req, res) => {
  const data = await leaveBalanceService.getMyLeaveBalance(req.user.organizationId, req.user._id, req.query);
  return success(res, data);
});

exports.updateLeaveBalance = catchAsync(async (req, res) => {
  const payload = updateBalanceSchema.parse(req.body);
  const leaveBalance = await leaveBalanceService.manualUpdate(req.user.organizationId, req.params.id, payload, req.user._id);
  return success(res, { leaveBalance });
});

// --- Accrual & Initialization ---

exports.initializeLeaveBalance = catchAsync(async (req, res) => {
  const payload = initializeBalanceSchema.parse(req.body);
  const financialYear = payload.financialYear || getFinancialYear();

  const leaveBalance = await leaveBalanceService.initializeUserBalance(req.user.organizationId, payload.userId, financialYear);
  return created(res, { leaveBalance });
});

exports.bulkInitializeLeaveBalances = catchAsync(async (req, res) => {
  const payload = bulkInitializeSchema.parse(req.body);
  const financialYear = payload.financialYear || getFinancialYear();

  const results = await leaveBalanceService.bulkInitialize(req.user.organizationId, financialYear, payload.carryForward, req.user._id);
  return success(res, results);
});

exports.accrueMonthlyLeave = catchAsync(async (req, res) => {
  const payload = accrueLeaveSchema.parse(req.body);
  const results = await leaveBalanceService.accrueMonthly(req.user.organizationId, payload.month, payload.year, req.user._id);
  return success(res, results);
});

// --- Reports ---

exports.getLeaveBalanceReport = catchAsync(async (req, res) => {
  const payload = reportQuerySchema.parse(req.query);
  const financialYear = payload.financialYear || getFinancialYear();

  const data = await leaveBalanceService.getReport(req.user.organizationId, financialYear, payload.departmentId);
  return success(res, data);
});

exports.getUtilizationTrends = catchAsync(async (req, res) => {
  const data = await leaveBalanceService.getTrends(req.user.organizationId, req.query.years);
  return success(res, data);
});
