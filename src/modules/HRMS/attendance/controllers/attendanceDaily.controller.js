...
exports.getAllDaily = catchAsync(async (req, res) => {
  const result = await repo.getList(req.user.organizationId, req.query);
  return success(res, result.data, 200, result.pagination);
});

exports.getDaily = catchAsync(async (req, res) => {
  const daily = await repo.getById(req.user.organizationId, req.params.id);
  if (!daily) throw new AppError('Record not found', 404);
  return success(res, { daily });
});

exports.getMyAttendance = catchAsync(async (req, res) => {
  const data = await attendanceDailyService.getMyAttendance(req.user.organizationId, req.user._id, req.query);
  return success(res, data);
});

exports.getTodayAttendance = catchAsync(async (req, res) => {
  // Existing method the user had delegated already
  const result = await attendanceDailyService.getTodayAttendance(req.user);
  return success(res, result);
});

exports.getAttendanceDashboard = catchAsync(async (req, res) => {
  const rawDate = parseQueryDate(req.query.date) || new Date();
  const data = await attendanceDailyService.getDashboard(req.user.organizationId, rawDate);
  return success(res, data);
});

exports.regularizeAttendance = catchAsync(async (req, res) => {
  const payload = regularizeSchema.parse(req.body);
  const daily = await attendanceDailyService.regularize(req.user.organizationId, req.params.id, payload, req.user._id);
  return success(res, { daily });
});

exports.bulkUpdateAttendance = catchAsync(async (req, res) => {
  const payload = bulkUpdateSchema.parse(req.body);
  const results = await attendanceDailyService.bulkUpdate(req.user.organizationId, payload.updates, req.user._id);
  return success(res, { updated: results.length, results });
});

exports.getAttendanceReport = catchAsync(async (req, res, next) => {
  const { fromDate, toDate, departmentId, userId } = dateRangeSchema.parse(req.query);
  ...
  const data = await attendanceDailyService.getReport(req.user.organizationId, from, to, departmentId, userId);
  return success(res, { period: { from: fromDate, to: toDate }, ...data });
});

exports.getAttendanceTrends = catchAsync(async (req, res) => {
  const months = parseInt(req.query.months) || 3;
  const trends = await attendanceDailyService.getTrends(req.user.organizationId, months);
  return success(res, { period: `${months} months`, trends });
});

exports.recalculateDaily = catchAsync(async (req, res) => {
  const payload = recalculateSchema.parse(req.body);
  const results = await attendanceDailyService.recalculateDaily(req.user.organizationId, payload.date);
  return success(res, { date: payload.date, processed: results.length });
});
...
exports.exportAttendance = catchAsync(async (req, res, next) => {
  ...
  const from = parseQueryDate(fromDate);
  const to = parseQueryDate(toDate);
  if (!isValidDateRange(from, to)) return next(new AppError('Invalid dates', 400));

  // The export data fetch is simple enough to execute directly or via repo
  const { dateRangeQuery } = require('../../../../core/utils/dateHelpers');
  ...
  return success(res, { count: data.length, records: data });
});
...
//  *
//  * Delegated to AttendanceDailyService to fetch today's state
//  * including cross-domain logic (Leaves, Holidays, Shifts).
//  */
