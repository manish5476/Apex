...
exports.createShift = catchAsync(async (req, res) => {
  const payload = createShiftSchema.parse(req.body);
  const shift = await shiftService.create(req.user.organizationId, payload, req.user._id);
  return created(res, { shift });
});

exports.getAllShifts = catchAsync(async (req, res) => {
...
exports.getShift = catchAsync(async (req, res, next) => {
  const shift = await repo.getById(req.user.organizationId, req.params.id);
  if (!shift) return next(new AppError('Shift not found', 404));
  return success(res, { shift });
});

exports.updateShift = catchAsync(async (req, res) => {
  const payload = updateShiftSchema.parse(req.body);
  const shift = await shiftService.update(req.user.organizationId, req.params.id, payload, req.user._id);
  return success(res, { shift });
});

exports.deleteShift = catchAsync(async (req, res) => {
  await shiftService.delete(req.user.organizationId, req.params.id, req.user._id);
  return noContent(res);
});

// --- Specialized Operations ---

exports.getShiftAssignments = catchAsync(async (req, res) => {
  const result = await repo.getShiftAssignments(req.user.organizationId, req.params.id, req.query);
  return success(res, { users: result.users }, 200, { 
    total: result.total, page: result.page, totalPages: result.totalPages 
  });
});

exports.calculateShiftHours = catchAsync(async (req, res) => {
  const payload = calcHoursSchema.parse(req.body);
  const result = shiftService.calculateHours(payload.startTime, payload.endTime, payload.breaks);
  return success(res, result);
});

exports.getShiftCoverage = catchAsync(async (req, res) => {
  const coverage = await shiftService.getCoverage(req.user.organizationId, req.query.date);
  return success(res, { coverage });
});

exports.cloneShift = catchAsync(async (req, res) => {
  const shift = await shiftService.clone(req.user.organizationId, req.params.id, req.user._id);
  return created(res, { shift });
});

exports.getShiftTimeline = catchAsync(async (req, res) => {
  const timeline = await shiftService.getTimeline(req.user.organizationId, req.query.date);
  return success(res, { timeline });
});

exports.validateShiftAssignment = catchAsync(async (req, res) => {
  const payload = validateAssignmentSchema.parse(req.body);
  const result = await shiftService.validateAssignment(req.user.organizationId, payload);
  return success(res, result);
});
