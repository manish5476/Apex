...
exports.createShiftGroup = catchAsync(async (req, res) => {
  const payload = createShiftGroupSchema.parse(req.body);
  const shiftGroup = await shiftGroupService.create(req.user.organizationId, payload, req.user._id);
  return created(res, { shiftGroup });
});

exports.getAllShiftGroups = catchAsync(async (req, res) => {
  const result = await repo.getList(req.user.organizationId, req.query);
  return success(res, result.data, 200, result.pagination);
});

exports.getShiftGroup = catchAsync(async (req, res, next) => {
  const shiftGroup = await repo.getById(req.user.organizationId, req.params.id);
  if (!shiftGroup) return next(new AppError('Shift group not found', 404));
  return success(res, { shiftGroup });
});

exports.updateShiftGroup = catchAsync(async (req, res) => {
  const payload = updateShiftGroupSchema.parse(req.body);
  const shiftGroup = await shiftGroupService.update(req.user.organizationId, req.params.id, payload, req.user._id);
  return success(res, { shiftGroup });
});

exports.deleteShiftGroup = catchAsync(async (req, res) => {
  await shiftGroupService.delete(req.user.organizationId, req.params.id, req.user._id);
  return noContent(res);
});

// --- Roster & Scheduling ---

exports.generateRotationSchedule = catchAsync(async (req, res) => {
  // Zod safely bounds the date range to 365 max days
  const payload = generateScheduleSchema.parse(req.body);
  
  const result = await shiftGroupService.generateRotationSchedule(
    req.user.organizationId, req.params.id, payload.startDate, payload.endDate, payload.userIds, req.user._id
  );
  return success(res, result);
});

exports.assignGroupToUsers = catchAsync(async (req, res) => {
  const payload = assignGroupSchema.parse(req.body);

  const result = await shiftGroupService.assignGroupToUsers(
    req.user.organizationId, req.params.id, payload.userIds, payload.startDate, payload.endDate, req.user._id
  );
  return success(res, result);
});

exports.getGroupAssignments = catchAsync(async (req, res) => {
  const result = await repo.getAssignments(req.user.organizationId, req.params.id, req.query);
  return success(res, { assignments: result.assignments }, 200, {
...
// const ShiftGroup = require('../../attendance/models/shiftGroup.model');
// const ShiftAssignment = require('../../attendance/models/shiftAssignment.model'); // FIX BUG-SG-C05
