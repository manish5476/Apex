 ...
 *   FIX #1 — checkCustomerBalanceIntegrity loaded ALL customers into Node.js
 *     memory and then looped. For large datasets this exhausts memory.
 *     Fixed: uses a single aggregation $lookup to join customer balances
 *     against AccountEntry, then filters mismatches in DB.
 *
 *   FIX #2 — detail endpoint queried AccountEntry.find({ referenceId: id })
 *     without an organizationId scope. Any user could read any invoice's
 *     ledger entries if they knew the ID.
 *
 *   FIX #3 — manualReconcilePayment didn't set installment.paidAt when
 *     marking an installment as paid.
 *
 *   FIX #4 — getReconciliationSummary passed raw organizationId (string)
 *     to an aggregate $match — must be ObjectId.
 */
...
/* ============================================================
   1. TOP MISMATCHES  (parallel integrity checks)
   ============================================================ */
exports.topMismatches = catchAsync(async (req, res, next) => {
  ...
  const [invoiceMismatches, paymentMismatches, customerMismatches] = await Promise.all([
    _checkInvoiceIntegrity(orgId),
    _checkPaymentIntegrity(orgId),
    _checkCustomerBalanceIntegrity(orgId), // FIX #1
  ]);
  ...
  res.status(200).json({ status: 'success', results: mismatches.length, data: { mismatches } });
});

/* ============================================================
   2. DETAIL DRILL-DOWN
   FIX #2: organizationId scoped on ledger query
   ============================================================ */
exports.detail = catchAsync(async (req, res, next) => {
  const { type, id } = req.query;
  const orgId = req.user.organizationId;

  let sourceDoc, ledgerEntries, analysis;

  if (type === 'invoice') {
    sourceDoc = await Invoice.findOne({ _id: id, organizationId: orgId });
    if (!sourceDoc) return next(new AppError('Invoice not found', 404));

    // FIX #2: scope to org
    ledgerEntries = await AccountEntry.find({ organizationId: orgId, referenceId: id, referenceType: 'invoice' });
    const arDebits = ledgerEntries.filter(e => e.debit > 0).reduce((s, e) => s + e.debit, 0);

    analysis = {
      docTotal: sourceDoc.grandTotal,
      ledgerTotal: arDebits,
      diff: parseFloat((sourceDoc.grandTotal - arDebits).toFixed(2)),
      isBalanced: Math.abs(sourceDoc.grandTotal - arDebits) < 0.01,
    };

  } else if (type === 'customer') {
    sourceDoc = await Customer.findOne({ _id: id, organizationId: orgId });
    if (!sourceDoc) return next(new AppError('Customer not found', 404));

    // FIX #2: scope to org
    const stats = await AccountEntry.aggregate([
      { $match: { organizationId: new mongoose.Types.ObjectId(orgId), customerId: new mongoose.Types.ObjectId(id) } },
      { $group: { _id: null, debit: { $sum: '$debit' }, credit: { $sum: '$credit' } } },
    ]);
    const calculatedBal = stats.length ? stats[0].debit - stats[0].credit : 0;

    analysis = {
      storedBalance: sourceDoc.outstandingBalance,
      calculatedBalance: calculatedBal,
      diff: parseFloat((sourceDoc.outstandingBalance - calculatedBal).toFixed(2)),
      isBalanced: Math.abs(sourceDoc.outstandingBalance - calculatedBal) < 0.01,
    };
    ledgerEntries = [];
  } else {
    return next(new AppError('Invalid type. Use: invoice, customer', 400));
  }

  res.status(200).json({ status: 'success', data: { sourceDoc, ledgerEntries, analysis } });
});
...
exports.getPendingReconciliations = catchAsync(async (req, res, next) => {
  const pending = await PendingReconciliation.find({
    organizationId: req.user.organizationId, status: 'pending',
  })
...
/* ============================================================
   4. MANUAL RECONCILE
   FIX #3: Sets installment.paidAt when marking as paid
   ============================================================ */
exports.manualReconcilePayment = catchAsync(async (req, res, next) => {
  ...
  if (!pending || String(pending.organizationId) !== String(req.user.organizationId)) {
    return next(new AppError('Reconciliation record not found', 404));
  }

  const emi = await EMI.findOne({ invoiceId: pending.invoiceId, organizationId: req.user.organizationId });
  ...
  const toProcess = Array.isArray(installments)
    ? [...installments].sort((a, b) => a - b)
    : emi.installments.map(i => i.installmentNumber).sort((a, b) => a - b);

  for (const instNum of toProcess) {
    if (remaining <= 0) break;
    const inst = emi.installments.find(i => i.installmentNumber === instNum);
    if (!inst || inst.paymentStatus === 'paid') continue;

    const pendingAmt = inst.totalAmount - inst.paidAmount;
    const toApply = Math.min(remaining, pendingAmt);

    inst.paidAmount += toApply;
    remaining -= toApply;
    inst.paymentStatus = inst.paidAmount >= inst.totalAmount ? 'paid' : 'partial';
    if (inst.paymentStatus === 'paid') inst.paidAt = new Date(); // FIX #3

    appliedInstallments.push({ installmentNumber: instNum, appliedAmount: toApply, newStatus: inst.paymentStatus });
  }

  if (remaining > 0) {
    emi.advanceBalance = (emi.advanceBalance || 0) + remaining;
    appliedInstallments.push({ type: 'advance', amount: remaining });
  }
  ...
  res.status(200).json({
    ...
    data: { reconciliation: pending, appliedInstallments, remainingAdvance: emi.advanceBalance || 0 },
  });
});

/* ============================================================
   5. RECONCILIATION SUMMARY
   FIX #4: Cast organizationId to ObjectId
   ============================================================ */
exports.getReconciliationSummary = catchAsync(async (req, res, next) => {
  const orgId = new mongoose.Types.ObjectId(req.user.organizationId); // FIX #4

  const [summary, emiSummary] = await Promise.all([
    PendingReconciliation.aggregate([
    ...
    EMI.aggregate([
      { $match: { organizationId: orgId } },
      { $unwind: '$installments' },
      { $group: { _id: '$installments.paymentStatus', count: { $sum: 1 }, totalAmount: { $sum: '$installments.totalAmount' }, paidAmount: { $sum: '$installments.paidAmount' } } },
    ]),
  ]);

  res.status(200).json({ status: 'success', data: { reconciliation: summary, installments: emiSummary } });
});
...
async function _checkInvoiceIntegrity(orgId) {
  return Invoice.aggregate([
    { $match: { organizationId: orgId, status: { $ne: 'cancelled' } } },
    { $lookup: { from: 'accountentries', localField: '_id', foreignField: 'referenceId', as: 'ledger' } },
    { $addFields: { ledgerSum: { $sum: '$ledger.credit' } } },
    { $project: { invoiceNumber: 1, grandTotal: 1, ledgerSum: 1, diff: { $abs: { $subtract: ['$grandTotal', '$ledgerSum'] } } } },
    { $match: { diff: { $gt: 0.05 } } },
    { $sort: { diff: -1 } },
    { $limit: 20 },
  ]);
...
async function _checkPaymentIntegrity(orgId) {
  return Payment.aggregate([
    { $match: { organizationId: orgId, status: 'completed' } },
    { $lookup: { from: 'accountentries', localField: '_id', foreignField: 'referenceId', as: 'ledger' } },
    { $addFields: { ledgerSum: { $sum: '$ledger.debit' } } },
    { $project: { referenceNumber: 1, amount: 1, ledgerSum: 1, diff: { $abs: { $subtract: ['$amount', '$ledgerSum'] } } } },
    { $match: { diff: { $gt: 0.05 } } },
    { $limit: 20 },
  ]);
}
...
/**
 * FIX #1: Pure aggregation — no memory load of all customers
 */
async function _checkCustomerBalanceIntegrity(orgId) {
  return Customer.aggregate([
    { $match: { organizationId: orgId } },
    {
      $lookup: {
        ...
        let: { custId: '$_id' },
        pipeline: [
          { $match: { $expr: { $and: [{ $eq: ['$organizationId', orgId] }, { $eq: ['$customerId', '$$custId'] }] } } },
          { $group: { _id: null, debit: { $sum: '$debit' }, credit: { $sum: '$credit' } } },
        ],
        as: 'ledgerStats',
      },
    },
    {
      $addFields: {
        ledgerBalance: {
          $subtract: [
            { $ifNull: [{ $arrayElemAt: ['$ledgerStats.debit', 0] }, 0] },
            { $ifNull: [{ $arrayElemAt: ['$ledgerStats.credit', 0] }, 0] },
          ],
        },
      },
    },
    {
      $project: {
        name: 1,
        storedBalance: '$outstandingBalance',
        realLedgerBalance: '$ledgerBalance',
        diff: { $subtract: ['$outstandingBalance', '$ledgerBalance'] },
      },
    },
    { $match: { $expr: { $gt: [{ $abs: '$diff' }, 1.0] } } },
    { $sort: { diff: -1 } },
    { $limit: 50 },
  ]);
}
