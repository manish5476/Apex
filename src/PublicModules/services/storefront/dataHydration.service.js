...
class DataHydrationService {
  /**
   * ============================================================
   * HYDRATE SECTIONS (The Orchestrator)
   * Takes static DB config -> Returns live data for the frontend
   * ============================================================
   * @param {Array} sections - The sections array from Page or Layout
  ...
  async hydrateSections(sections, organizationId) {
    ...
    const hydrationTasks = sections.map(async (section) => {
      // A. Skip inactive sections immediately
      if (section.isActive === false) return null;

      try {
        // B. Delegate to the Registry
        // The Registry will look up the correct Resolver (Product, Nav, Map) and execute it.
        // It returns a copy of the section with a populated `.data` property.
        return await SectionRegistry.resolveSectionData(section, organizationId);

      } catch (error) {
        // C. Safety Net (Pro Feature)
        // If one section fails (e.g. Map API error), log it but DO NOT crash the page.
        console.error(`[Hydration Failure] Section ${section.id} (${section.type}):`, error.message);
        
        // Return the section with an error flag so the Frontend can hide it gracefully
        return { ...section, data: null, error: true };
      }
    });

    // 3. Execute Concurrently (Pro Performance)
    // We use allSettled logic implicitly here because our try/catch above ensures 
    // promises don't reject. We just filter the results.
