...
/**
 * Hydrates Storefront sections with live data from database/engine.
 */
class DataHydrationService {
    async hydrateSections(sections = [], organizationId) {
        if (!Array.isArray(sections) || !sections.length) return [];
        
        // Execute hydration for all sections in parallel
        return Promise.all(sections.map(section => this.hydrateSingleSection(section, organizationId)));
    }
    ...
    async hydrateSingleSection(section, organizationId) {
        ...
        try {
            // 1. Navigation / Navbar Hydration
            if (section.type.includes('navbar')) {
                hydrated.data = await this.getNavigationLinks(organizationId);
                return hydrated;
            }

            // 2. Product-based Sections (Grid, Slider)
            if (['product_grid', 'product_slider'].includes(section.type)) {
                if (section.dataSource === 'smart_rule' && section.config?.ruleType) {
                    hydrated.data = await SmartRuleEngine.executeAdHocRule(section.config, organizationId);
                } else if (section.dataSource === 'manual' && section.manualData?.productIds) {
            ...
            if (section.type === 'category_grid') {
                hydrated.data = await Master.find({ 
                    organizationId, 
                    type: 'category', 
                    isActive: true 
                }).limit(8).select('name slug imageUrl').lean();
                return hydrated;
            }

            return hydrated;
        } catch (err) {
            logger.error(`Hydration Error [${section.type}]: ${err.message}`);
            return hydrated;
        }
    }

    async getNavigationLinks(organizationId) {
        const pages = await StorefrontPage.find({ 
            organizationId, 
            status: 'published', 
            isPublished: true 
        }).select('name slug isHomepage').sort({ isHomepage: -1, name: 1 }).lean();
