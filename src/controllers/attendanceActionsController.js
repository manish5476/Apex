...
exports.submitRegularization = catchAsync(async (req, res, next) => {
    ...
    // 1. Validation
    if (!dayjs(targetDate).isValid()) return next(new AppError('Invalid date format YYYY-MM-DD', 400));
    
    // Prevent fixing future dates
    if (dayjs(targetDate).isAfter(dayjs())) return next(new AppError('Cannot regularize future dates', 400));
...
exports.decideRegularization = catchAsync(async (req, res, next) => {
    ...
    // 2. Permission Check (Must be Admin or Manager)
    // Assuming you have a helper like req.user.isAdmin() or role checks
    // if (req.user.role.name !== 'Admin') ... (Add your specific RBAC here)
    ...
    try {
        ...
        if (status === 'approved') {
            ...
            // Mark as Manual Override in DB (Optional flag)
