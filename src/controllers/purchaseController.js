...
// --- HELPER: POST PURCHASE ---
async function postPurchase(purchase, session, req) {
  for (const item of purchase.items) {
    const product = await Product.findById(item.productId).session(session);
    if (product) {
      let inv = product.inventory.find(i => i.branchId.toString() === purchase.branchId.toString());
      if (!inv) { product.inventory.push({ branchId: purchase.branchId, quantity: 0 }); inv = product.inventory[product.inventory.length-1]; }
      inv.quantity += item.quantity;
      await product.save({ session });
    }
  }
  await Supplier.findByIdAndUpdate(purchase.supplierId, { $inc: { outstandingBalance: (purchase.grandTotal - (purchase.paidAmount||0)) } }, { session });

  const invAcc = await Account.findOne({ organizationId: purchase.organizationId, name: 'Inventory Asset' }).session(session);
  const apAcc = await Account.findOne({ organizationId: purchase.organizationId, name: 'Accounts Payable' }).session(session);
  if (!invAcc || !apAcc) throw new AppError('Critical: Missing Inventory/AP Accounts', 500);

  // Dr Inventory, Cr AP
  await AccountEntry.create([
      { organizationId: purchase.organizationId, branchId: purchase.branchId, accountId: invAcc._id, date: purchase.purchaseDate, debit: purchase.grandTotal, credit: 0, description: `Purchase #${purchase.invoiceNumber}`, referenceType: 'purchase', referenceId: purchase._id, createdBy: req.user._id },
      { organizationId: purchase.organizationId, branchId: purchase.branchId, accountId: apAcc._id, supplierId: purchase.supplierId, date: purchase.purchaseDate, debit: 0, credit: purchase.grandTotal, description: `Bill #${purchase.invoiceNumber}`, referenceType: 'purchase', referenceId: purchase._id, createdBy: req.user._id }
  ], { session });
}
...
exports.createPurchase = catchAsync(async (req, res, next) => {
  ...
  try {
    const purchaseArr = await Purchase.create([{ ...req.body, organizationId: req.user.organizationId, branchId: req.user.branchId, createdBy: req.user._id, status: req.body.status || 'draft' }], { session });
    const purchase = purchaseArr[0];
    if (purchase.status === 'received') await postPurchase(purchase, session, req);
...
exports.updatePurchase = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    if (oldStatus === 'draft' && purchase.status === 'received') await postPurchase(purchase, session, req);
    await session.commitTransaction();
    res.status(200).json({ status: "success", data: { purchase } });
  } catch (err) { await session.abortTransaction(); next(err); } finally { session.endSession(); }
});

// 🔒 SECURE CANCEL (Return to Supplier)
exports.cancelPurchase = catchAsync(async (req, res, next) => {
  ...
  try {
    const purchase = await Purchase.findOne({ _id: req.params.id, organizationId: req.user.organizationId }).session(session);
    if (!purchase) throw new AppError("Purchase not found", 404);
    if (purchase.status !== 'received') throw new AppError("Only received purchases can be cancelled", 400);

    // 1. Reverse Inventory
    for (const item of purchase.items) {
        const product = await Product.findById(item.productId).session(session);
        if (product) {
            const inv = product.inventory.find(i => i.branchId.toString() === purchase.branchId.toString());
            if (inv) {
                if (inv.quantity < item.quantity) throw new AppError(`Cannot return ${product.name}: stock too low`, 400);
                inv.quantity -= item.quantity;
                await product.save({ session });
            }
        }
    ...
    // 3. GL Reversal (Dr AP, Cr Inventory)
    const invAcc = await Account.findOne({ organizationId: purchase.organizationId, name: 'Inventory Asset' }).session(session);
    const apAcc = await Account.findOne({ organizationId: purchase.organizationId, name: 'Accounts Payable' }).session(session);
    
    if (invAcc && apAcc) {
        await AccountEntry.create([
            { organizationId: purchase.organizationId, branchId: purchase.branchId, accountId: apAcc._id, supplierId: purchase.supplierId, date: new Date(), debit: purchase.grandTotal, credit: 0, description: `Cancel Pur #${purchase.invoiceNumber}`, referenceType: 'purchase', referenceId: purchase._id, createdBy: req.user._id },
            { organizationId: purchase.organizationId, branchId: purchase.branchId, accountId: invAcc._id, date: new Date(), debit: 0, credit: purchase.grandTotal, description: `Cancel Pur #${purchase.invoiceNumber}`, referenceType: 'purchase', referenceId: purchase._id, createdBy: req.user._id }
        ], { session });
    }
...
exports.deletePurchase = catchAsync(async (req, res, next) => {
  const purchase = await Purchase.findOne({ _id: req.params.id, organizationId: req.user.organizationId });
  if (!purchase) return next(new AppError('No purchase found', 404));
  if (purchase.status !== 'draft') return next(new AppError('Cannot delete received purchase. Use Cancel.', 400));
