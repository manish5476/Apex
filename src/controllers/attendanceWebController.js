...
const Branch = require('../models/branchModel'); // You need this to get office coords
...
exports.markAttendance = catchAsync(async (req, res, next) => {
    ...
    if (user.attendanceConfig.enforceGeoFence) {
        ...
        if (!branch || !branch.location?.coordinates) {
            // Fallback: If branch has no location, we might log a warning or block
            // For now, let's assume strict mode:
