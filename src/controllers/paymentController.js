...
// /**
//  * Create Payment (inflow or outflow)
//  *
//  * Body expected:
//  * {
//  *   type: 'inflow' | 'outflow',
...
//  *   paymentDate?, referenceNumber?, transactionId?, bankName?, remarks?
//  * }
//  */
// exports.createPayment = catchAsync(async (req, res, next) => {
//   const {
...
//     return next(new AppError('customerId is required for inflow payments', 400));
//   }
//   if (type === 'outflow' && !supplierId) {
//     return next(new AppError('supplierId is required for outflow payments', 400));
//   }
...
//   try {
//     // Create payment document (scoped to organization and branch)
...
//     // If payment is not completed (e.g., pending/failed), we still persist but do not change balances
//     const shouldAffectBalances = payment.status === 'completed';

//     // Handle inflow (customer receipt)
//     if (type === 'inflow' && shouldAffectBalances) {
//       // 1) Update Invoice (if provided)
//       if (invoiceId) {
...
//         // Increment paidAmount and recalc balance/paymentStatus
...
//       // 2) Update Customer outstandingBalance
//       await Customer.findOneAndUpdate(
//         { _id: customerId, organizationId: req.user.organizationId },
//         { $inc: { outstandingBalance: -amount } }, // customer owes less
//         { session }
//       );

//       // 3) Create Ledger entry (credit the bank/cash account, debit customer)
//       await Ledger.create(
//         [
//           {
//             organizationId: req.user.organizationId,
...
//             customerId,
//             paymentId: payment._id,
//             type: 'credit', // money received, reduces customer's debit
//             amount,
//             description: `Payment received${invoiceId ? ' for Invoice ' + invoiceId : ''}`,
//             accountType: 'customer',
//             createdBy: req.user._id,
//           },
//         ],
...
//     // Handle outflow (supplier payment)
//     if (type === 'outflow' && shouldAffectBalances) {
//       // 1) Update Purchase (if provided)
//       if (purchaseId) {
...
//       // 2) Update Supplier outstandingBalance
//       await Supplier.findOneAndUpdate(
//         { _id: supplierId, organizationId: req.user.organizationId },
//         { $inc: { outstandingBalance: -amount } }, // we owe less
//         { session }
//       );

//       // 3) Create Ledger entry (debit bank/cash, credit supplier)
//       await Ledger.create(
//         [
//           {
//             organizationId: req.user.organizationId,
...
//             supplierId,
//             paymentId: payment._id,
//             type: 'debit', // money paid out
//             amount,
//             description: `Payment made to supplier${purchaseId ? ' for Purchase ' + purchaseId : ''}`,
//             accountType: 'supplier',
//             createdBy: req.user._id,
//           },
//         ],
...
// exports.updatePayment = factory.updateOne(Payment); // careful: changing status may need business logic
// exports.deletePayment = factory.deleteOne(Payment);

// /* =============== Convenience endpoints =============== */

// /**
//  * Get payments for a given customer (org-scoped)
//  */
// exports.getPaymentsByCustomer = catchAsync(async (req, res, next) => {
//   const { customerId } = req.params;
//   const payments = await Payment.find({
...
// /**
//  * Get payments for a given supplier (org-scoped)
//  */
// exports.getPaymentsBySupplier = catchAsync(async (req, res, next) => {
//   const { supplierId } = req.params;
//   const payments = await Payment.find({
