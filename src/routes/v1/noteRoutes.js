...
router.get(
  ...
  noteController.getKnowledgeGraph // New: Network graph nodes/links
);
...
router.post(
  ...
  noteController.restoreFromTrash // Specific restore for soft-deleted items
);

router.delete(
  "/trash/empty",
  checkPermission(PERMISSIONS.NOTE.DELETE),
  noteController.emptyTrash
);
...
// ==============================================================================
// 3. ID-DEPENDENT ROUTES (Must come last to avoid capturing other paths)
// ==============================================================================
...
router.post(
  "/:id/link",
  checkPermission(PERMISSIONS.NOTE.WRITE),
  noteController.linkNote // New: Bidirectional linking
);
...
router.patch(
  "/:id/restore",
  checkPermission(PERMISSIONS.NOTE.WRITE),
  noteController.restoreNote // Legacy: restore from 'archived' status
);
...
router.delete(
  "/:id/permanent",
  checkPermission(PERMISSIONS.NOTE.DELETE),
  noteController.hardDeleteNote // Permanent delete
);
...
router.delete("/:id", checkPermission(PERMISSIONS.NOTE.DELETE), noteController.deleteNote); // Soft delete
