...
router
  .route('/')
  .get(authController.restrictTo('superadmin', 'admin'), ledgerController.getAllLedgers);

// Single ledger entry
router
  .route('/:id')
  .get(ledgerController.getLedger)
  .delete(authController.restrictTo('platform-admin'), ledgerController.deleteLedger);
