...
// --- In-Memory State (Local to this server instance) ---
// Note: In a multi-server setup, these Maps only track users connected to THIS specific server.
// Global presence (who is online across ALL servers) is usually handled via Redis Sets, 
// but for now, we will rely on the Redis Adapter to route messages correctly.
...
function init(server, options = {}) {
  ...
  const { cors, jwtSecret, pingTimeout = 30000 } = options;
  ...
  // 🚀 REDIS ADAPTER SETUP (The Fix for Scalability)
  // This allows Server A to send messages to users on Server B
  ...
  (async () => {
    try {
      ...
      console.warn('⚠️ Redis Adapter failed to connect (falling back to memory):', err.message);
    }
  })();
  ...
  io.on('connection', (socket) => {
    ...
    // 2. Broadcast Online Status (To all servers via Redis)
    // We join a personal room so we can message this user easily from controllers
    socket.join(`user:${userId}`);
    
    // We join the Org room for broadcasts
    socket.join(`org:${orgId}`);

    // If this is the *first* socket for this user on *this* server, we track them locally
    // Note: In a perfect world, we'd check Redis for global presence, but this is a good hybrid.
    if (getSocketIdsForUser(userId).length === 1) {
      addOrgOnlineUser(orgId, userId);
      // Emit to everyone in the Org room (across all servers)
    ...
    socket.on('joinChannel', async ({ channelId } = {}) => {
      ...
      try {
        ...
        // ideally, you might cache channel existence in Redis to avoid DB hits on every join
        
        socket.join(`channel:${channelId}`);
        socket.joinedChannels.add(channelId);
        addUserToChannel(channelId, userId);
        
        // Broadcast to channel (Redis handles cross-server)
        socket.to(`channel:${channelId}`).emit('userJoinedChannel', { userId, channelId });
        
        // Send list of users (Only from local state - limitation of hybrid approach)
        // For perfect global lists, you'd need Redis Sets for channel members.
    ...
    socket.on('sendMessage', async (payload = {}) => {
        ...
        if (!channelId || (!body && !attachments)) return;
        ...
        } catch(e) { console.error(e); socket.emit('error', { code: 'SERVER_ERROR' }); }
    });

    socket.on('typing', ({ channelId, typing }) => {
        if(channelId) socket.to(`channel:${channelId}`).emit('userTyping', { userId, channelId, typing: !!typing });
    });

    socket.on('markRead', async ({ channelId, messageIds }) => {
        // ... (Keep existing logic) ...
        if(channelId && messageIds?.length) {
             await Message.updateMany({ _id: { $in: messageIds }, readBy: { $ne: userId } }, { $push: { readBy: userId } });
             socket.to(`channel:${channelId}`).emit('readReceipt', { userId, channelId, messageIds });
        }
    });
...
// --- Global Emitters (Work across all servers via Redis) ---
...
function forceDisconnectUser(userId) {
  if (!io) return;
  // This broadcasts a disconnect signal to ALL servers.
  // The server holding the socket for 'user:{userId}' will receive it and disconnect the client.
...
// //     console.log(`🔌 Socket Connected: ${socket.id}`);
// //     console.log(`➡️ Joining Org Room: org:${orgId}`); // <--- CHECK THIS LOG
// //     // register socket
// //     addSocketForUser(userId, socket.id);
...
// // function emitToOrg(organizationId, event, payload) {
// //   if (!io) {
// //     console.error('❌ SOCKET ERROR: io is not initialized yet! Cannot emit.');
// //     return;
// //   }
// //   console.log(`📢 Emitting to Room: org:${organizationId}`); // Debug Log
// //   io.to(`org:${organizationId}`).emit(event, payload);
// // }
